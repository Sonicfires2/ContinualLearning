"""Maximally Interfered Retrieval replay baseline."""
...
from src.replay import MIRTraceLogger, ReservoirReplayBuffer, select_mir_replay_items
...
class MIRReplayBaselineConfig(RandomReplayBaselineConfig):
    """Configuration for the ER-MIR replay baseline."""

    method_name: str = "mir_replay"
    run_name: str = "mir_replay_baseline"
    mir_candidate_size: int = 128
    mir_virtual_lr: float | None = None


@dataclass(frozen=True)
class MIRReplayBaselineRun:
...
def _experiment_run_config(
    *,
    ...
    return ExperimentRunConfig(
        ...
        method={
            "description": "experience replay with maximally interfered retrieval",
            "uses_replay": True,
            "forgetting_aware": True,
            "selection_policy": "er_mir_mi1",
        },
        replay={
            "enabled": True,
            ...
            "sampling_policy": "maximally_interfered_retrieval",
            ...
            "mir_score": "post_virtual_update_loss_minus_pre_update_loss",
        },
        signals={
    ...
    *,
        ...
        for _epoch in range(trainer_config.epochs_per_task):
            for batch in train_loader:
                ...
                if len(replay_buffer) > 0:
                    ...
                    mir_trace.record(selections, candidate_count=len(candidate_items))
    ...
    return ContinualTrainingResult(
        ...
        method_metrics={
            ...
            "mir": mir_trace.summary(),
        },
    )


def run_mir_replay_baseline(config: MIRReplayBaselineConfig) -> MIRReplayBaselineRun:
    """Run the ER-MIR baseline and save experiment artifacts."""

    if config.target_key != "original_class_id":
        raise ValueError(
            "MIR replay defaults to class-incremental original_class_id targets; "
    ...
    artifacts = save_experiment_artifacts(
        ...
        extra_metadata={
            ...
            "research_role": "strong_replay_comparator",
            "mir_variant": "er_mir_mi1",
        },
        extra_json_artifacts={
            **(
            ...
            "mir_trace": mir_trace.to_json_payload(),
        },
    )
    return MIRReplayBaselineRun(
        result=result,
        artifacts=artifacts,
        run_config=run_config,
    )
...
def load_config(path: str | Path) -> MIRReplayBaselineConfig:
    """Load a MIR replay config from YAML."""

    raw = OmegaConf.to_container(OmegaConf.load(path), resolve=True)
    if not isinstance(raw, dict) or "mir_replay_baseline" not in raw:
        raise ValueError("config must contain a mir_replay_baseline section")
    section = raw["mir_replay_baseline"]
    if "hidden_dims" in section:
        section["hidden_dims"] = tuple(section["hidden_dims"])
    return MIRReplayBaselineConfig(**section)
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(description="Run the ER-MIR replay baseline")
    ...
    try:
        ...
        return 2
    print(f"Saved MIR replay artifacts to: {run.artifacts.run_dir}")
    return 0
